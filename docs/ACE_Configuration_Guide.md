...
2. **精华提取** ✨ (v1.0)：智能压缩和提取对话精华
   - 一次对话通常只生成 **1 条** 精炼的 insight（200-800 字符）
   - 多次修改的代码只保留**最终成功版本**
   - 自动过滤琐碎操作（ls、cat 等），但必须记录未解决的问题
   - 减缓上下文膨胀 **80%**（从 2000 字符/对话降至 400 字符）
3. **知识提取**：提取有价值的模式、策略和技巧
4. **存储**：将提取的知识存储为结构化的 "bullets"
5. **检索**：下次相关查询时，自动加载相关历史知识
6. **注入**：将上下文注入到对话中，帮助 AI 给出更好的建议
