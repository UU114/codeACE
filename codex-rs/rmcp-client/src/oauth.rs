...
//!
//! If the keyring is not available or fails, we fall back to CODEACE_HOME/.credentials.json which is consistent with other coding CLI agents.
...
mod tests {
    ...
    impl TempCodexHome {
        fn new() -> Self {
            ...
            let dir = tempdir().expect("create CODEACE_HOME temp dir");
            unsafe {
                std::env::set_var("CODEACE_HOME", dir.path());
            }
            Self {
                _guard: guard,
                _dir: dir,
    ...
    impl Drop for TempCodexHome {
        fn drop(&mut self) {
            unsafe {
                std::env::remove_var("CODEACE_HOME");
            }
        }
    }
